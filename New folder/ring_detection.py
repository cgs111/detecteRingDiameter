...
def ring_detection(img):
    ...
    circles = cv2.HoughCircles(blurImg, cv2.HOUGH_GRADIENT, 1, otsuTrethsold, minRadius=30, maxRadius=250)
    ...
    t = 0
    while t < 16:
        if grayImg[y1, x1 + t] < 50 and grayImg[y1, x1 + t + 1] < 50 and grayImg[y1, x1 + t + 2] < 50:
            if grayImg[y1, x1 + t + 3] < 50 and grayImg[y1, x1 + t + 4] < 50 and grayImg[y1, x1 - t + 5] < 50:
                break
    ...
    t = 0
    while t < 16:
        if grayImg[y2, x2 - t] < 50 and grayImg[y2, x2 - t - 1] < 50 and grayImg[y2, x2 - t - 2] < 50:
            if grayImg[y2, x2 - t - 3] < 50 and grayImg[y2, x2 - t - 4] < 50 and grayImg[y2, x2 - t - 5] < 50:
                x2 = x2 - t + 1
                break
