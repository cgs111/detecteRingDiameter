...
def card_detection(img):
    ...
    imgCanny = cv2.Canny(grayImg, otsuTrethsold - 10, otsuTrethsold + 10, None, 3)
    color_imgCanny = cv2.cvtColor(imgCanny, cv2.COLOR_GRAY2BGR)
    minLineLength = 100
    maxLineGap = 20
    linesP = cv2.HoughLinesP(imgCanny, 1, np.pi / 180, int(otsuTrethsold), None, minLineLength, maxLineGap)
    x = []
